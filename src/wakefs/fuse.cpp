...
// Staging debug log macro - only logs if debug_log is non-null
#define STAGING_LOG(fmt, ...) \
  do { if (debug_log) { fprintf(debug_log, fmt, ##__VA_ARGS__); fflush(debug_log); } } while(0)
...
static bool collect_result_metadata(const std::string daemon_output, const struct timeval &start,
  ...
  // If staging_files is absent, fuse-waked is running in legacy mode (--no-cas)
...
bool run_in_fuse(fuse_args &args, int &status, std::string &result_json) {
  ...
  // Open CAS store for storing job outputs
  // CAS root is the parent of the blobs directory (e.g., ".cas" from ".cas/blobs")
