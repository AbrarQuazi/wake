...
static std::optional<cas::ResolvedPath> do_resolve(const char* file) {
  auto mode =
      use_cas_hashes() ? cas::ResolvePathMode::StoreInCas : cas::ResolvePathMode::HashOnly;
  auto policy = use_cas_hashes() ? cas::SpecialFilePolicy::Reject
                                 : cas::SpecialFilePolicy::LegacyExoticHash;

  auto result = cas::resolve_path(file, mode, policy);
  if (!result) {
...
int main(int argc, char** argv) {
  ...
  for (auto& identity : identities) {
    if (identity) {
      if (emit_type) {
        std::cout << identity->type << ":" << identity->hash << std::endl;
      } else {
